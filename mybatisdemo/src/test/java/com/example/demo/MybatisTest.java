...
public class MybatisTest {
	...
	public static void findEmployeerById(int id) {
		...
		try {
			...
			Employeer employeer = (Employeer) session.selectOne(
					"com.example.demo.EmployeerMapper.findEmployeerByID", 1);
			if (employeer == null)
	...
	public static void addEmployeer(Employeer employeer){
		...
		try {
			session = sqlSessionFactory.openSession();
			//返回值是记录条数
			int resultCount = session.insert("com.example.demo.EmployeerMapper.addEmployeer", employeer );
	...
	public static void deleteEmployeer(int id){
		...
		try {
			session = sqlSessionFactory.openSession();
			//返回值是记录条数
			int resultCount=session.delete("com.example.demo.EmployeerMapper.deleteEmployeer",id);
	...
	public static void updateEmployeer(Employeer employeer){
		...
		try {
			session = sqlSessionFactory.openSession();
			session.update("com.example.demo.EmployeerMapper.updateEmployeer",employeer);
			session.commit() ;
		} finally {
			session.close();
		}
